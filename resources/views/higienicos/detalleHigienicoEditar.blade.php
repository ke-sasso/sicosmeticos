...
@endif
		<div class="row">
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
              ...
              <input type="hidden" name="idHigienico" id="idHigienico" value="{{$hig->idHigienico}}" />
                                          
                                          @include('higienicos.panelesEdicion.generales-editar')

                                          @include('higienicos.panelesEdicion.clasificacion-editar')                

                                          @include('panelesGenerales-editar.propietario-editar')

                                          @include('panelesGenerales-editar.profesional-editar')

                                          @include('higienicos.panelesEdicion.marca-editar')

                                          @include('higienicos.panelesEdicion.formula-editar')

                                          @include('higienicos.panelesEdicion.presentacionesHig')
...
//BUSQUEDA DE CLASIFICACION
  ...
  var idC=$('#idClas').val();
  var nombreC=$('#nombreClas').val();

  var clasSelect=$('#clasSelect').selectize({
        valueField: 'idClas',
        labelField: 'nombreClas',
        searchField: ['idClas','nombreClas'], 
        maxOptions: 10,
            options: [{idClas: idC, nombreClas: nombreC}],
            create: false,
        render:{
          option: function(item, escape) {
                    return '<div>'+ escape(item.nombreClas) +'</div>';
                  }
            },
         load: function(query, callback) {
                    if (!query.length) return callback();    
            $.ajax({
              url:"{{route('buscar.clasificacion')}}",
              type: 'GET',
                    dataType: 'json',
                    data: {
                            q: query, 
     ...
     $('#clasSelect').selectize()[0].selectize.setValue(idC); 
...
$('#agregarFormula').click(function(){
  ...
  var inputPor='';
   ...
   $.ajax({
        url:"{{ route('buscarFormulaHigjaxPorId') }}",
        data:{_token: '{{ csrf_token() }}',
                  id: idDen
            },
        type: 'post',
        success: function(data){
            var nuevaFila="";
            nuevaFila+="<tr><input type='hidden' name='idDenominacion[]' value='"+data[0].idDenominacion+"'><input type='hidden' name='numeroCAS[]' value='"+data[0].numeroCAS+"'><input type='hidden' name='denominacionINCI[]' value='"+data[0].nombreSustancia+"'><input type='hidden' name='porcentaje[]' value='"+por+"'><td>"+data[0].numeroCAS+"</td><td>"+data[0].nombreSustancia+"</td><td>"+por+"%</td><td><a class='btn btn-xs btn-danger borrar'><i class='fa fa-times' aria-hidden='true'></i></a></td></tr>";
...
$('#generalesSave').click(function(){
	...
	$.ajax({
		data: $('#frmGeneralesHig').serialize() +"&NoRegistro="+idHig+"&_token="+_token,
		url: "{{route('editarGenenalesHig')}}",
...
$('#clasificacionSave').click(function (){
  var clasificacion=$('#clasSelect').val();
  var uso=$('#uso').val();
	$.ajax({
		data: "uso="+uso+"&clasificacion="+clasificacion+"&NoRegistro="+idHig+"&_token="+_token,
		url: "{{route('editarClasHig')}}",
...
$('#marcaSave').click(function (){
	...
	$.ajax({
		data: "marca="+marca+"&NoRegistro="+idHig+"&_token="+_token,
		url: "{{route('editarMarcaHig')}}",
...
$('#propietarioSave').click(function(){
	$.ajax({
		data: $('#frmPropietario').serialize() +"&NoRegistro="+idHig+"&_token="+_token,
		url: "{{route('editarPropietarioHig')}}",
...
$('#profesionalSave').click(function (){
	$.ajax({
		data: $('#frmProfesional').serialize() +"&NoRegistro="+idHig+"&_token="+_token,
		url: "{{route('editarProfesionalHig')}}",
...
$('#formulaSave').click(function (){
	$.ajax({
		data: $('#formulaFrm').serialize() +"&NoRegistro="+idHig+"&_token="+_token,
		url: "{{route('editarFormulaHig')}}",
...
console.log("hola tonos");
	...
	if(tipo=='HIG')
		url="{{route('editarTonosHig')}}";
	else 
		url="{{route('editarTonosHig')}}";

	$.ajax({
		data: $('#tonosFrm').serialize() +"&NoRegistro="+idHig+"&_token="+_token,
		url: url,
...
//	console.log("hola");
	...
	if(tipo=='HIG')
		url="{{route("editarFraganciaHig")}}";
	else 
		url="{{route("editarFraganciaHig")}}";
	
	$.ajax({
		data: $('#fraganciaFrm').serialize() +"&NoRegistro="+idHig+"&_token="+_token,
		url: url,
...
$('#fabSave').click(function (){
	var tipo =$('#clasificacionProducto').val();
	if(tipo=='HIG')
		url="{{route("editarFabHig")}}";
	else 
		url="{{route("editarFabHig")}}";
	
	
	$.ajax({
		data: $('#fabFrm').serialize() +"&NoRegistro="+idHig+"&_token="+_token,
		url: url,
...
$('#disSave').click(function (){
	var tipo =$('#clasificacionProducto').val();
	if(tipo=='HIG')
		url="{{route("editarDisHig")}}";
	else 
		url="{{route("editarDisHig")}}";
	
	
	$.ajax({
		data: $('#disFrm').serialize() +"&NoRegistro="+idHig+"&_token="+_token,
		url: url,
...
$('#ImpSave').click(function (){
	var tipo =$('#clasificacionProducto').val();
	if(tipo=='HIG')
		url="{{route("editarImpHig")}}";
	else 
		url="{{route("editarImpHig")}}";
	
	
	$.ajax({
		data: $('#impFrm').serialize() +"&NoRegistro="+idHig+"&_token="+_token,
		url: url,
...
$('#savePres').click(function(){
  ...
  clas=valor;
  higienico=$('#idHigienico').val();
  token=$('input[name="_token"]').val();

  $.ajax({
          url: "{{ route('actualizar.presentacionHig') }}",
           data: { idHigienico:higienico, 
                  _token :token,
          ...
          success:  function (r){
            ...
            var nuevaFila="";
                    nuevaFila+="<tr><td>"+total+"</td><td>"+r.data['presentaciones'][0].textoPresentacion+"</td><td><button type='button' class='btn btn-xs btn-danger borrarPres' data-idpres='"+r.data['presentaciones'][0].idPresentacion+"'><i class='fa fa-times' aria-hidden='true'></i></button> </td></tr>";
...
$(document).on('click', '.borrarPres', function (event) {
      ...
      var idHigienico=$('#idHigienico').val();
      console.log(fila);
      $.ajax({
              data: {idPresentacion:idPresentacion, idHigienico:idHigienico},
              url: '{{route("borrar.presentacionesHig")}}',
              type: 'get',
              beforeSend: function() {
                $('body').modalmanager('loading');
              },
